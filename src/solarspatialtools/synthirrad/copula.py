...
def _space_time_copula(N, D, csi, cdf, p, seed=None):
    """
    ...
    csi : float
        The CSI values corresponding to the CDF, shape (n_csi,). This is used
...
def downscale(times, Epos, Npos, cloud_dir, cloud_spd, mean_csi, params,
              seed=42, scale=True, noneg=True):
    """
    ...
    D = spatial.spacetime_distances(Epos, Npos, times, cloud_spd, cloud_dir)

    M = _space_time_copula(1, D, csi, cdf, p, seed=seed)
    M = M.reshape(1, Epos.shape[0], times.shape[0]).transpose([0,2,1])
...
def __matlab_compare():
    ...
    for i in range(len(hcsi)):
        ...
        cm = downscale(times, Epos, Npos, cd[i], cs[i], hcsi[i], param,
...
def __spatial_compare():
    ...
    c = downscale(times, Epos, Npos, cd, cs, hcsi, param, scale=True,
                  noneg=True)
